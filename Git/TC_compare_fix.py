...
def run_vqe(ansatz, n_qubits, n_layers, seed, n_steps, bonds, H_matrix, E_exact):
    ...
    def step_fn(params, opt_state):
        ...
        return new_params, opt_state, loss, grads   # <-- 顺便把grads也返回, 不新增编译

    last_grads = None
    for _ in range(n_steps):
        params, opt_state, loss, last_grads = step_fn(params, opt_state)

    # ---- 训练结束后的梯度范数: 直接复用最后一次循环里已经算好的 grads ----
    # 注意: 这是"最后一次参数更新前"那一点的梯度, 跟"最终params"差一步之遥,
    # 对诊断"是否停在驻点"这个用途完全够用, 而且不产生任何新的JIT编译
    final_grad_norm = float(np.linalg.norm(np.array(last_grads)))
